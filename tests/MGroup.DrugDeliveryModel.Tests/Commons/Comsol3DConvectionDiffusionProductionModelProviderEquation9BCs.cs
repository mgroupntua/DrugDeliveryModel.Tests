...
{
    public class Comsol3DConvectionDiffusionProductionModelProviderEquation9BCs
	{
        ...
        public Comsol3DConvectionDiffusionProductionModelProviderEquation9BCs(double[] convectionCoeff,
        ...
        public  Model CreateModelFromComsolFile(string filename)
		{
            ...
            {
                if (Math.Abs(0.1 - node.Z) < 1E-9) topNodes.Add(node);
			...
			{
				dirichletBCs[i] = new NodalUnknownVariable(node, ConvectionDiffusionDof.UnknownVariable, 50);
				i++;
			}
