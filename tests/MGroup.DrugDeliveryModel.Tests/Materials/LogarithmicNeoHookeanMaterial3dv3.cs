...
namespace MGroup.DrugDeliveryModel.Tests.Materials
{
    ...
    /// </summary>
    public class LogarithmicNeoHookeanMaterial3dv2 : IContinuumMaterial3DDefGrad
	{
		...
		public LogarithmicNeoHookeanMaterial3dv2(double mi, double kappa)
		{
			this.mi = mi;
			lamda = (kappa - ((double)2 / 3) * mi);
		}

		public LogarithmicNeoHookeanMaterial3dv2(){}
		...
		public double[] UpdateConstitutiveMatrixAndEvaluateResponse(double[] DefGradVec )
		{
			...
			var SpkNH = (mi * 0.5) * I_1_st - (mi) * ((double)1 / J_3) * (J_3_st) + lamda * Math.Log(J_3) * ((double)1 / J_3) * J_3_st;
			...
			for (int i1 = 0; i1 < 6; i1++)
			{
				...
				{
					Cons[i1, i2] = -(mi) * ((double)1 / J_3) * (J_3_stst[i1, i2])
						+ mi * Math.Pow(J_3, -2) * J_3_st_vec[i1] * J_3_st_vec[i2]
		...
		public object Clone() => new LogarithmicNeoHookeanMaterial3dv2() { mi= this.mi,lamda =this.lamda};
