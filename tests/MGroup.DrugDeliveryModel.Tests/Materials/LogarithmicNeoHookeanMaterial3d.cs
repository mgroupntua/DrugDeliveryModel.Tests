...
namespace MGroup.DrugDeliveryModel.Tests.Materials
{
    ...
    /// </summary>
    public class LogarithmicNeoHookeanMaterial3d : IContinuumMaterial3DDefGrad
	{
		...
		public LogarithmicNeoHookeanMaterial3d(){}
		...
		public double[] UpdateConstitutiveMatrixAndEvaluateResponse(double[] DefGradVec )
		{
			...
			var SpkNH = (mi * 0.5) * I_1_st - (2 * mi + lamda) * ((double)1 / I_3) * (0.5) * (0.5 * I_3_st) + ((0.5) * lamda * 0.5) * I_3_st;
			var Spk_vec = new double[] { SpkNH[0, 0], SpkNH[1, 1], SpkNH[2, 2], SpkNH[0, 1], SpkNH[1, 2], SpkNH[2, 0] };

			//var Cons2 = (2 * (-(2 * mi + lamda) * 0.5 * ((double)1 / I_3) + 0.5 * lamda) * 0.25) * I_3_stst - (2 * (2 * mi + lamda) * 0.5 * (-1) * Math.Pow(I_3, -2) * 0.25) * (I_3_st_vec.TensorProduct(I_3_st_vec));
		...
		public object Clone() => new LogarithmicNeoHookeanMaterial3d() { mi= this.mi,lamda =this.lamda};
